...
class Refinery(tk.Tk, BinillaWidget, RefineryCore):
    ...
    def queue_add_all_maps(self, e=None):
        ...
        starting_engine = self.active_engine_name

        # Add all maps to the queue
        for engine in self.maps_by_engine:
            if engine == ACTIVE_INDEX: continue
            self.set_active_engine(engine)
            for map in sorted(self.maps_by_engine[engine]):
                if map == ACTIVE_INDEX: continue
        ...
        self.set_active_engine(starting_engine)
